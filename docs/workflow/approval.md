...
| 驳回策略 | 说明 | 适用场景 |
| --- | --- | --- |
| 终止流程 | 直接结束流程，实例状态变为「已驳回」 | 通用场景，驳回后停止流转 |
...
发起人可在「运行中」实例上执行撤回，且流程全局设置需要允许撤回。

撤回后的影响：

1. 流程实例状态变为「已撤回」
2. 所有未处理的任务自动标记为「已跳过」
3. 触发 `instance.withdrawn` 事件，可订阅该事件做后续处理

审批人已通过的任务支持在流程仍运行时撤回已办。撤回后该任务回到待处理状态，流程回到对应节点重新等待审批。

## 转办 / 委派

### 转办

转办会把当前待办直接交给目标用户处理，当前处理人退出该任务。转办链路会记录最初处理人和经手人，禁止把任务转回曾经经手的人员，避免 A → B → A 之类的循环。

### 委派

委派会把当前任务交给目标用户先给出处理建议。被委派人通过或拒绝后，系统生成一条回执任务回到最早的委派人，由原委派人进行最终通过或拒绝。

| 行为 | 说明 |
| --- | --- |
| 委派反馈 | 被委派人填写建议，任务状态记录为通过或拒绝 |
| 回执任务 | 系统为最早委派人生成同节点待办 |
| 最终确认 | 最早委派人根据回执意见完成最终审批 |

多次委派只回到最早委派人确认。外部审批回调不会触发委派回执逻辑。

## 协办

审批人可在自己的待办任务上发起协办，邀请其他用户提供处理意见。协办人只回复意见，不代替审批人完成审批。

| 操作 | 说明 |
| --- | --- |
| 发起协办 | 当前任务处理人选择协办人并填写说明 |
| 我的协办 | 协办人查看分配给自己的协办请求 |
| 回复协办 | 协办人提交意见，供审批人参考 |

## 加签 / 减签

允许审批人在不修改流程定义的前提下，临时拉人参与或移除已加签的审批人。前端入口在「待办审批」页面，需要节点允许加/减签且「加签」按钮启用。
...
| `parallel`（并加签） | 加签人与原审批人并行处理，按加签时选择的会签/或签方式汇总 | `[加签-并]` |

### 减签

只能减掉「由加签产生的待办任务」（`comment` 以 `[加签-` 开头且 `status = pending` 或 `waiting`），减签后任务标记为 `skipped`。
...
| 能力 | 说明 |
| --- | --- |
| 批量通过 | 对多个待办任务执行批量审批通过，并返回每条成功/失败结果 |
| 批量驳回 | 对多个待办任务执行批量审批驳回，并返回每条成功/失败结果 |
| 常用语 | 待办审批页可维护常用审批意见，按排序快速选择 |
| 附件 | 操作按钮可要求上传附件，审批处理时随意见提交 |
| 手写签名 | 节点开启手写签名后，通过时需要提交签名 |
| 审批人自选 | 下一节点配置为审批人自选时，当前审批人处理时指定下一节点审批人 |

## 退回

审批人可在节点允许回退且「退回」按钮启用时，将流程退回到前序审批/办理节点。前端可选择一个或多个可退回节点，后端按流程顺序取最早的目标节点重新生成任务。

| 退回模式 | 说明 |
| --- | --- |
| 重新执行 | 从退回目标节点开始按流程重新执行 |
| 回到原节点 | 退回目标节点完成后回到当前节点继续处理 |

## 转办 / 委派轨迹

### 任务的处理人轨迹

`workflow_tasks` 使用以下字段维护处理人轨迹：

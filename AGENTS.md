...
- **Service 层**：业务逻辑、数据映射、前置校验从路由中提取到 `src/services/xxx.service.ts`（已覆盖所有路由）。命名约定：`mapXxx`（数据映射，纯函数）、`ensureXxx`（前置校验，抛 `AppError`）。**禁止**在 service 中调用 `c.json()`、访问 Hono 上下文、使用 `console.*`；路由 handler 只负责取参数、调 service、返回 HTTP 响应。DB 唯一约束异常（PG 错误码 `23505`）仍在路由的 `try-catch` 中捕获并返回 400。`AppError` 定义在 `src/lib/errors.ts`，由 `src/index.ts` 的全局 `onError` 统一转为标准 JSON 错误响应。
